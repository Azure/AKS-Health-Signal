...
type HealthSignalSpec struct {
	...
	//
	// Deprecated: this field is no longer read by the AKS Resource Provider and
	// has no effect. It will be removed in v1beta1.
	//
	// A timeout stored here can only be enforced once the HealthSignal exists,
	// so a provider that never creates one is never timed out — the failure mode
	// this field is meant to protect against. The waiting deadline is therefore
	// owned by the RP and configured per provider on UpgradeGatePolicy
	// (upgrade.aks.io/v1alpha1), which is evaluated before the health check
	// starts and so also bounds a provider that stays silent.
	//
	// A provider that wants to fail sooner than its deadline should report a
	// condition with status "False" rather than setting this field.
	// +optional
